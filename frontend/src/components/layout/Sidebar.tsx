...
import { Home, BookOpen, Settings, LayoutDashboard, X } from "lucide-react"
...
export function Sidebar({ role, userName, userEmail, courses, open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-[var(--color-border)] bg-white p-4 transition-transform duration-200 lg:static lg:z-auto lg:translate-x-0",
      ...
      >
        <div className="mb-6 flex items-center justify-between px-1">
          ...
          <button className="lg:hidden text-[var(--color-ink-soft)]" onClick={onClose} aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="mb-4 flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--color-slate-light)]/60 p-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-[var(--color-ink-soft)]">{userEmail}</p>
          </div>
        </div>

        <nav className="flex-1 space-y-1 overflow-y-auto thin-scroll">
          {role === "student" ? (
