...
class CourseRetrievalTool:
    ...
    def retrieve(self, course_id: str, query: str, top_k: int = 4) -> list[RetrievedContext]:
        ...
        # Defense in depth: strictly enforce course isolation.
        # Drop anything missing metadata or tagged for a different course.
        filtered = []
        for r in results:
            metadata = r.metadata if isinstance(r.metadata, dict) else {}
            chunk_course = metadata.get("course_id")
            if self.strict_isolation:
                if chunk_course == course_id:
                    filtered.append(r)
            else:
                if chunk_course in (None, course_id):
                    filtered.append(r)
        return filtered
